...
def seo(name,url):
    ...
    with open('ip权重查询结果.txt','a',encoding='utf-8') as y:
...
def exp(name1):
    ...
    # except:
        ...
        with open('ip权重失败结果.txt',mode='a',encoding='utf-8') as o:
            o.write(name1+'\n')
...
def main():
    multithreading(exp, [], "iip.txt", 15)  # 默认15线程
    print("全部check完毕,请查看当前目录下的ip权重.txt")
